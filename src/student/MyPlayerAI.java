...
/**
 * The class for a class that represents a AI player in a Railroad Barons game.
 *
 * @author Lyan Ye, Joey Zhen
 */
public class MyPlayerAI implements Player {
     ...
     */
    public MyPlayerAI(Baron baron, RailroadBarons railroadBarons) {
    ...
    public void MostToMost() {
        List<Node> path;
        for (Station start: eastmost) {
            for (Station end : westmost) {
                if (canReachDFS(this.graph, start.getName(), end.getName())) {
                    path = (List<Node>)buildPathDFS(this.graph, start.getName(), end.getName());
                    List<Node> nei = this.graph.getNeighbors(this.graph.getNode(end.getName()));
                    int check = 0;
                    for (Node n : nei) {
                        if (!n.getName().equals(end.getName())) {
                            check = 1;
                        }
                    }
                    if (path.size() >= 3) {
        ...
        for (Station s: northmost) {
            for (Station e: southmost) {
                if (canReachDFS(this.graph, s.getName(), e.getName())) {
                    ...
                    List<Node> nei = this.graph.getNeighbors(this.graph.getNode(e.getName()));
                    int check = 0;
                    for (Node n : nei) {
